...
export function LeadForm({ intent, context, onIntentChange }: { intent: LeadIntent; context: LeadContext; onIntentChange: (intent: LeadIntent) => void }) {
  ...
  return <section id="posaljite-cjenik" className="lead-section" aria-labelledby="lead-title" tabIndex={-1}>
    <div className="lead-copy"><span className="app-label">NE MORATE ZNATI TEHNIČKE DETALJE</span><h2 id="lead-title">{intent === 'plugin' ? 'Zatražite ponudu za plugin' : intent === 'implementation' ? 'Zatražite ponudu za plugin + implementaciju' : 'Pošaljite što imate. Mi ćemo predložiti najkraći put.'}</h2><p>{intent === 'plugin' ? 'Plugin za vaš CMS — 49,90 €. Pošaljite platformu i što imate; odgovaramo s konkretnom ponudom, bez automatskog linka.' : intent === 'implementation' ? 'Plugin i tehničko postavljanje na vaš web — 89,90 € zajedno (ili 49,90 € samo implementacija). Pošaljite što imate.' : 'Možete poslati datoteku, adresu weba ili samo opisati gdje danas držite cijene. Odgovorit ćemo konkretno, bez obveze.'}</p><div className="lead-price"><strong>{intent === 'plugin' ? 'Plugin 49,90 €' : intent === 'implementation' ? 'Plugin + implementacija 89,90 €' : 'Konzultacija'}</strong><span>{intent === 'plugin' ? 'Ugradnja / priprema plugin rješenja. Bez besplatnog trajnog hostinga na NEPAR domeni.' : intent === 'implementation' ? 'Plugin i postavljanje na postojeći web. Pošaljete materijale — mi predložimo put.' : 'Pregled vašeg slučaja i preporuka sljedećeg koraka.'}</span></div></div>
