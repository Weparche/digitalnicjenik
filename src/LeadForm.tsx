...
export function LeadForm({ intent, context, onIntentChange }: { intent: LeadIntent; context: LeadContext; onIntentChange: (intent: LeadIntent) => void }) {
  ...
  return <section id="posaljite-cjenik" className="lead-section" aria-labelledby="lead-title" tabIndex={-1}>
    <div className="lead-copy"><span className="app-label">NE MORATE ZNATI TEHNIČKE DETALJE</span><h2 id="lead-title">{intent === 'plugin' ? 'Pomoć s pluginom ili aktivacija linka' : 'Pošaljite što imate. Mi ćemo predložiti najkraći put.'}</h2><p>{intent === 'plugin' ? 'Imate CSV/XML za ugradnju u CMS, ili želite trajni Publisher link s dashboardom (magic-link prijava). Pošaljite platformu i što ste već probali.' : 'Možete poslati datoteku, adresu weba ili samo opisati gdje danas držite cijene. Odgovorit ćemo konkretno, bez obveze.'}</p><div className="lead-price"><strong>{intent === 'plugin' ? 'Plugin / trajni link' : 'Implementacija od 129 €'}</strong><span>{intent === 'plugin' ? 'Tehnička pomoć oko ugradnje ili aktivacija self-serve Publisher tenanta. Demo sandbox nije trajni hosting.' : 'Tehnička priprema, postavljanje na postojeći web i prva godina Publishera.'}</span></div></div>
    <form className="lead-form" onSubmit={submit} encType="multipart/form-data">
      ...
      {(context.checkerStatus || context.sourceFilename) && <div className="lead-context"><strong>Kontekst koji možete poslati uz upit</strong>{context.checkerStatus && <span>Provjera weba: {context.checkerStatus}</span>}{context.discoveredUrls?.map((url) => <span key={url}>{url}</span>)}{context.sourceFilename && <span>{context.sourceFilename} · {context.itemCount ?? 0} stavki · {context.blockerCount ?? 0} za dopunu</span>}</div>}
