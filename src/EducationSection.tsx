...
export function EducationSection({ onLead }: { onLead: (intent: LeadIntent) => void }) {
  return <section className="education-section" aria-labelledby="education-title">
    ...
    <div className="education-cta"><div><strong>Želite konkretnu ponudu?</strong><span>Pošaljite datoteku, URL ili fotografiju postojećeg cjenika.</span></div><div><button className="app-button app-button-primary" type="button" onClick={() => onLead('implementation')}>Plugin + implementacija 89,90 €</button><button className="app-button app-button-light" type="button" onClick={() => onLead('plugin')}>Plugin 49,90 €</button></div></div>
  </section>
}
